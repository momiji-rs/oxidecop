...
macro_rules! assignment_write {
    ($self:expr, $node:expr) => {{
        ...
        $self.value_used_offsets.insert($node.value().location().start_offset());
    }};
}
...
impl<'pr, 'a> Visit<'pr> for Cops<'a> {
    ...
    fn visit_return_node(&mut self, node: &ruby_prism::ReturnNode<'pr>) {
        ...
        if let Some(args) = node.arguments() {
            for a in args.arguments().iter() {
                ...
                self.value_used_offsets.insert(a.location().start_offset());
            }
        }
        let kw = node.keyword_loc();
    ...
    fn visit_call_node(&mut self, node: &ruby_prism::CallNode<'pr>) {
        ...
        if let Some(r) = node.receiver() {
            ...
            self.value_used_offsets.insert(r.location().start_offset());
            self.visit(&r);
        }
        if self.hot.semicolon {
        ...
        if let Some(a) = node.arguments() {
            ...
            for arg in a.arguments().iter() {
                self.value_used_offsets.insert(arg.location().start_offset());
                if track_args {
...
pub fn lint(src: &[u8], cfg: &Config, eng: &Engine, rel_path: &str) -> LintResult {
    ...
    // Lint/Syntax: mirrors `Commissioner#investigate` — when the file
    // doesn't parse cleanly, ONLY this cop's diagnostics get reported; every
    // check below would otherwise run against a nonsense partial tree.
    if cops.check_syntax(&result) {
        let mut offenses = cops.offenses;
        offenses.sort_by(|a, b| (a.line, a.col, a.cop).cmp(&(b.line, b.col, b.cop)));
        return LintResult { offenses, fixes: cops.fixes };
    }
...
mod tests {
    ...
    fn detect_spec_cases() {
        ...
        let r = lint_all(src, &cfg);
        assert_eq!(r.offenses[0].message, "Use `detect` instead of `select.first`.");
        assert_eq!(apply_fixes(src, r.fixes), "[1, 2, 3].detect { |i| i % 2 == 0 }\n");
        assert_eq!(
